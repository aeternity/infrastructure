...
- `HOST` - Pass IP (or a comma separated list) to limit the inventory to the specific host(s) (ansible's `-i`)
...
Example for deploying by specifying custom node config:
```bash
